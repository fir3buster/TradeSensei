...
const updateApplicantRecord = async (req, res) => {
  try {
    ...
    if ("priceOut" in req.body) updateApplicant.priceOut = req.body.priceOut;
...
const getAllManagersFinalRecords = async (req, res) => {
  try {
    const allManagersFinalRecords = await ManagerFinalRecordModel.find().sort({
      staffId: 1,
    });
...
const getGeneralManagersRecordsByApplicantId = async (req, res) => {
  try {
    ...
    const ManagersFinalRecordsByApplicantId =
      await ManagerFinalRecordModel.find({
        generalManagerRecord: allGeneralManagerRecordsByApplicantId[0]._id,
      });
    res.json({
